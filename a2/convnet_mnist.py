...
with tf.variable_scope('conv1') as scope:
    ...
    kernel = tf.get_variable(name='kernel', shape=[5, 5, 1, 32], initializer=tf.truncated_normal_initializer())

    # create kernel variable of dimension [5, 5, 1, 32]
    # use tf.truncated_normal_initializer()

    bias = tf.get_variable(name='bias', shape=[32], initializer=tf.constant_initializer(0.0))
...
# you should know how to do this too
    ...
    entrophy = tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=logits)
    loss = tf.reduce_mean(entrophy, name='loss')
# Step 7: define training op
# using gradient descent with learning rate of LEARNING_RATE to minimize cost
# don't forget to pass in global_step
